...
function publicPlayerList(lobby) {
  // Nur das, was alle sehen dürfen — keine Rollen, keine Wörter
  return lobby.players.map(p => ({ id: p.id, name: p.name, connected: p.connected }));
}
...
io.on('connection', (socket) => {
  ...
  socket.on('cast-vote', ({ targetId }) => {
    ...
    io.to(lobby.code).emit('vote-progress', { votedCount, totalCount: lobby.players.length });
