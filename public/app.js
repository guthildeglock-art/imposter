...
function renderCustomWords(words, isHost) {
  localCustomWords = words;
  const listEl = document.getElementById('customWordsList');
  const hint = document.getElementById('customWordsHint');
  if (words.length === 0) {
    listEl.innerHTML = '';
    ...
    listEl.innerHTML = words.map(w => `
      <div class="player-row" style="justify-content:space-between;">
        <span>${escHtml(w.word)} <span class="muted small" style="color:${w.byColor}">${escHtml(w.byName)}</span></span>
        ${isHost ? `<button onclick="removeCustomWord('${escHtml(w.word)}')" style="width:auto;min-height:unset;padding:2px 8px;font-size:13px;border-color:#e07b6a;color:#e07b6a;">✕</button>` : ''}
      </div>`).join('');
  }
}

function removeCustomWord(word) {
  socket.emit('remove-custom-word', { word });
}
...
function addCustomWord() {
  ...
  if (!val) return;
  socket.emit('add-custom-word', { word: val });
  input.value = '';
}
...
socket.on('custom-word-added', ({ word, byName, byColor, customWords }) => {
  renderCustomWords(customWords.map((w, i) => {
    const existing = localCustomWords.find(x => x.word === w);
    return existing || { word: w, byName: i === customWords.length - 1 ? byName : '?', byColor: i === customWords.length - 1 ? byColor : '#999' };
  }), isHost);
  // Merken wer was vorgeschlagen hat (wir bauen eine Map)
  if (!window._wordAuthors) window._wordAuthors = {};
  window._wordAuthors[word] = { byName, byColor };
  renderCustomWordsFromServer(customWords);
});

socket.on('custom-word-removed', ({ customWords }) => {
  renderCustomWordsFromServer(customWords);
});

function renderCustomWordsFromServer(words) {
  if (!window._wordAuthors) window._wordAuthors = {};
  const enriched = words.map(w => ({
    word: w,
    byName: (window._wordAuthors[w] || {}).byName || '?',
    byColor: (window._wordAuthors[w] || {}).byColor || '#999'
  }));
  renderCustomWords(enriched, isHost);
}
...
document.getElementById('reviewVoteBtn').addEventListener('click', () => {
  socket.emit('host-vote-or-describe', { action: 'vote' });
});
document.getElementById('reviewDescribeAgainBtn').addEventListener('click', () => {
  socket.emit('host-vote-or-describe', { action: 'describe-again' });
});

function renderReviewScreen(data) {
  const listEl = document.getElementById('reviewHintsList');
  const hints = data.submittedHints || {};
  const entries = Object.entries(hints);
  if (entries.length === 0) {
    listEl.innerHTML = '<p class="muted small">Keine Hinweise eingegeben</p>';
  } else {
    listEl.innerHTML = entries.map(([id, h]) => {
      const color = colorOf(id);
      return `<div class="vote-results-row"><span style="color:${color};font-weight:500">${escHtml(h.name)}</span><span>${escHtml(h.text)}</span></div>`;
  ...
  document.getElementById('reviewHostActions').style.display = isHost ? 'block' : 'none';
  document.getElementById('reviewGuestWaiting').style.display = isHost ? 'none' : 'block';
  showScreen('screen-describe-review');
}
...
function bindAllChatInputs() {
  [
    ...
    ['chatInputReview','chatSendBtnReview'],
...
socket.on('describe-review', data => {
  addChatMsg('', '', '📋 Alle haben ihr Hinweis-Wort eingegeben!', true);
