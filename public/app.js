// app.js — Online-Modus: verbindet sich per Socket.io mit dem Server.
...
if (joinFromUrl) {
  document.getElementById('joinCodeInput').value = joinFromUrl.toUpperCase().slice(0,4);
...
function showQrCode(code) {
  ...
  target.innerHTML = '';
  new QRCode(target, {
    text: window.location.origin + window.location.pathname + '?join=' + code,
    width: 160, height: 160, colorDark: '#1a1a1a', colorLight: '#ffffff'
  });
...
// ---- Chat (Online) ----
// Alle Chat-Panels teilen denselben Log — wird beim Bildschirmwechsel in den neuen Container kopiert.
let chatLog = [];
...
socket.on('lobby-state', state => {
  ...
  // Meine eigene Farbe merken (für Chat)
  ...
  state.players.forEach(p => {
    ...
    nameSpan.style.display = 'flex'; nameSpan.style.alignItems = 'center';
  ...
  if (state.phase === 'lobby') {
    ...
    if (!isMe) {
      document.getElementById('guessWaitingText').textContent = nameOf(state.votedOutId) + ' darf jetzt das Wort erraten…';
    } else {
...
socket.on('your-role', data => {
  ...
  addChatMsg('', '', '🃏 Alle haben ihr Wort gesehen — diskutiert los!', true);
